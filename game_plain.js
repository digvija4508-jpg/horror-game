...
let localBones = null; // Cached references to local character bones to eliminate render loop lag
let clock = new THREE.Clock();

// Game entities
let mapModel = null;
let playerModel = null;
let cachedAvatarGLTF = null; // Cached GLTF structure of the character
...
// --- Procedural Avatar Generator (Mecha Chameleon Style) ---
function createProceduralStickman(color = 0x00f0ff) {
    const rootGroup = new THREE.Group();
    rootGroup.name = "stickman";

    // Neon colors: Emissive Basic materials so they are visible in dark first-person POV
    const jointMat = new THREE.MeshBasicMaterial({ color: color });
    const limbMat = new THREE.MeshBasicMaterial({ color: color });
    const eyeMat = new THREE.MeshBasicMaterial({ color: 0xff007f }); // Neon pink eyes

    // Helper to create joint spheres (caps)
    function makeJoint(size = 0.05) {
    ...
    // Helper to create limb cylinders (pivot at top)
    function makeLimb(length = 0.2, radius = 0.02) {
        const geom = new THREE.CylinderGeometry(radius, radius, length, 6);
        geom.translate(0, -length / 2, 0); // shift pivot to top end of cylinder
    ...
    // --- Build Character Skeleton Hierarchy ---
    // Pelvis / Hips (Root Node)
    const pelvis = makeJoint(0.06);
    pelvis.name = "pelvis";
    rootGroup.add(pelvis);

    // Spine
    const spine = makeLimb(0.35, 0.03);
    spine.name = "spine";
    spine.position.set(0, 0.05, 0);
    pelvis.add(spine);

    // Chest
    const chest = makeJoint(0.06);
    chest.name = "chest";
    chest.position.set(0, -0.35, 0); // Positioned relative to top end of spine
    spine.add(chest);

    // Neck & Head
    const neck = makeLimb(0.08, 0.015);
    neck.name = "neck";
    neck.position.set(0, 0.03, 0);
    chest.add(neck);

    const head = makeJoint(0.085);
    head.name = "head";
    head.position.set(0, -0.08, 0);
    neck.add(head);

    // Glow pink chameleon eyes
    const leftEye = makeJoint(0.02);
    leftEye.material = eyeMat;
    leftEye.position.set(-0.06, 0.02, -0.04);
    head.add(leftEye);

    const rightEye = makeJoint(0.02);
    rightEye.material = eyeMat;
    rightEye.position.set(0.06, 0.02, -0.04);
    head.add(rightEye);

    // --- Arms ---
    // Left Shoulder -> Upper Arm -> Elbow -> Forearm -> Hand
    const lShoulder = makeJoint(0.045);
    lShoulder.name = "lShoulder";
    lShoulder.position.set(-0.09, 0.0, 0);
    chest.add(lShoulder);

    const lUpperArm = makeLimb(0.24, 0.02);
    lUpperArm.name = "lUpperArm";
    lShoulder.add(lUpperArm);

    const lElbow = makeJoint(0.035);
    lElbow.name = "lElbow";
    lElbow.position.set(0, -0.24, 0);
    lUpperArm.add(lElbow);

    const lForearm = makeLimb(0.22, 0.018);
    lForearm.name = "lForearm";
    lElbow.add(lForearm);

    const lHand = makeJoint(0.03);
    lHand.name = "lHand";
    lHand.position.set(0, -0.22, 0);
    lForearm.add(lHand);

    // Right Shoulder -> Upper Arm -> Elbow -> Forearm -> Hand (Flashlight Hand)
    const rShoulder = makeJoint(0.045);
    rShoulder.name = "rShoulder";
    rShoulder.position.set(0.09, 0.0, 0);
    chest.add(rShoulder);

    const rUpperArm = makeLimb(0.24, 0.02);
    rUpperArm.name = "rUpperArm";
    rShoulder.add(rUpperArm);

    const rElbow = makeJoint(0.035);
    rElbow.name = "rElbow";
    rElbow.position.set(0, -0.24, 0);
    rUpperArm.add(rElbow);

    const rForearm = makeLimb(0.22, 0.018);
    rForearm.name = "rForearm";
    rElbow.add(rForearm);

    const rHand = makeJoint(0.03);
    rHand.name = "rHand";
    rHand.position.set(0, -0.22, 0);
    rForearm.add(rHand);

    // --- Legs ---
    // Left Hip -> Thigh -> Knee -> Calf -> Foot
    const lHip = makeJoint(0.05);
    lHip.name = "lHip";
    lHip.position.set(-0.08, -0.03, 0);
    pelvis.add(lHip);

    const lThigh = makeLimb(0.32, 0.024);
    lThigh.name = "lThigh";
    lThigh.rotation.z = 0.05;
    lHip.add(lThigh);

    const lKnee = makeJoint(0.04);
    lKnee.name = "lKnee";
    lKnee.position.set(0, -0.32, 0);
    lThigh.add(lKnee);

    const lCalf = makeLimb(0.30, 0.02);
    lCalf.name = "lCalf";
    lKnee.add(lCalf);

    const lFoot = makeJoint(0.035);
    lFoot.name = "lFoot";
    lFoot.position.set(0, -0.30, 0.03);
    lCalf.add(lFoot);

    // Right Hip -> Thigh -> Knee -> Calf -> Foot
    const rHip = makeJoint(0.05);
    rHip.name = "rHip";
    rHip.position.set(0.08, -0.03, 0);
    pelvis.add(rHip);

    const rThigh = makeLimb(0.32, 0.024);
    rThigh.name = "rThigh";
    rThigh.rotation.z = -0.05;
    rHip.add(rThigh);

    const rKnee = makeJoint(0.04);
    rKnee.name = "rKnee";
    rKnee.position.set(0, -0.32, 0);
    rThigh.add(rKnee);

    const rCalf = makeLimb(0.30, 0.02);
    rCalf.name = "rCalf";
    rKnee.add(rCalf);

    const rFoot = makeJoint(0.035);
    rFoot.name = "rFoot";
    rFoot.position.set(0, -0.30, 0.03);
    rCalf.add(rFoot);

    // Save reference lookup map of all joints
    rootGroup.userData.bones = {
        pelvis, spine, chest, neck, head,
        lShoulder, lUpperArm, lElbow, lForearm, lHand,
        rShoulder, rUpperArm, rElbow, rForearm, rHand,
        lHip, lThigh, lKnee, lCalf, lFoot,
        rHip, rThigh, rKnee, rCalf, rFoot
    };
...
function init() {
    ...
    // 2. Camera Setup (Split View: Left 1st-person, Right 3rd-person)
    camera = new THREE.PerspectiveCamera(60, (window.innerWidth / 2) / window.innerHeight, 0.005, 100);
    thirdPersonCamera = new THREE.PerspectiveCamera(60, (window.innerWidth / 2) / window.innerHeight, 0.01, 100);
    ...
    // Create a beautiful flat grid testing plain (no foliage obstacles or hills)
    const gridHelper = new THREE.GridHelper(100, 100, 0xff3f3f, 0x44444c);
    ...
    mapModel = plane;
    collidableMeshes = [plane];

    // Hardcode map dimensions for spawn / placement
    mapCenterX = 0;
    mapCenterZ = 0;

    // Force local player to spawn in the center of our grid
    spawnPosition.set(0, 0.025, 0);
    resetPlayerPosition();
    
    // Snap campfire to the flat testing plain
    snapCampfireToGround();

    // Set up local player model
    playerModel = createProceduralStickman(0x00f0ff); // Neon cyan mecha stickman

    // Set up player group to rotate/translate cleanly
    playerGroup = new THREE.Group();
    playerGroup.add(playerModel);
    
    // Position first-person camera statically at eye level, shifted slightly forward to clear head/neck geometry
    ...
    // Scale player dynamically to configured height (1.135 units is the height of our procedural stickman)
    const scaleFactor = CONFIG.playerHeight / 1.135;
    ...
    localBones = playerModel.userData.bones;

    // Setup dummy animator/mixer structure if other scripts query them
    mixer = null; 
    ...
    function showSplitOverlays(show) {
        ...
        if (divider) {
            if (show) divider.classList.remove('hidden');
...
function updatePlayerAnimations(deltaTime) {
    ...
    const bones = playerModel.userData.bones;
    if (!bones) return;

    const time = clock.getElapsedTime();
    
    // Check if player is moving based on active keyboard/joystick movement inputs
    ...
    let speedMultiplier = isSprinting ? 2.2 : 1.0;
    let walkCycle = time * 12 * speedMultiplier;

    if (!isGrounded && !flyMode) {
        // Jump Pose: Lift thighs, bend knees, and lift arms up
        bones.lThigh.rotation.x = -0.5;
        bones.rThigh.rotation.x = -0.5;
        bones.lKnee.rotation.x = 0.8;
        bones.rKnee.rotation.x = 0.8;
        bones.lShoulder.rotation.x = 2.0;
        bones.rShoulder.rotation.x = 2.0;
        bones.lElbow.rotation.x = -0.5;
        bones.rElbow.rotation.x = -0.5;
        bones.spine.rotation.x = 0.1;
    } else if (isMoving) {
        // Walk/Run gait: smooth opposition swing
        const swing = Math.sin(walkCycle);
        const swingDouble = Math.sin(walkCycle * 2);

        // Legs swing in opposition
        bones.lThigh.rotation.x = swing * 0.6;
        bones.rThigh.rotation.x = -swing * 0.6;

        // Knees bend on the backswing
        bones.lKnee.rotation.x = (swing < 0 ? -swing : 0) * 0.8;
        bones.rKnee.rotation.x = (swing > 0 ? swing : 0) * 0.8;

        // Spine bobs up and down and tilts forward slightly
        bones.spine.rotation.x = 0.15 + swingDouble * 0.05;
        ...
        bones.lThigh.rotation.set(0, 0, 0.05);
        bones.rThigh.rotation.set(0, 0, -0.05);
        bones.lKnee.rotation.set(0, 0, 0);
        bones.rKnee.rotation.set(0, 0, 0);
...
function animate() {
    ...
    // 2. Flashlight & Arm Posing: Bind flashlight to right hand and tilt/bob the arm naturally in POV
    if (playerModel && torchMesh && localBones) {
        const rightHand = localBones.rightHand;
        if (rightHand && torchMesh.parent !== rightHand) {
            rightHand.add(torchMesh);
            // Position/rotate flashlight model nicely relative to the palm
            torchMesh.position.set(0, 0.003, 0.002);
            torchMesh.rotation.set(Math.PI / 2, 0, 0);
        }

        // Apply hand-held arm overrides for local player
        const rightArm = localBones.rightArm;
        const rightForeArm = localBones.rightForeArm;
        
        if (rightArm && rightForeArm) {
            // Calculate natural arm bobbing from the walking cycle
            const isMoving = (keys['w'] || keys['s'] || keys['a'] || keys['d'] || 
                              keys['arrowup'] || keys['arrowdown'] || keys['arrowleft'] || keys['arrowright'] ||
                              (isTouchDevice && joystickDir.lengthSq() > 0));
            const isSprinting = ((keys['w'] || keys['arrowup']) && (keys['q'] || keys['shift'])) || (isTouchDevice && mobileSprinting && joystickDir.lengthSq() > 0);
            ...
            if (isMoving && isGrounded && !flyMode) {
                ...
                bobX = Math.sin(bobTime) * 0.02;
                bobY = Math.cos(bobTime * 0.5) * 0.01;
            }

            // Sway: delay right arm rotations slightly on horizontal/vertical looking
            const yawDiff = targetYaw - yaw;
            const pitchDiff = targetPitch - pitch;
            const swayY = THREE.MathUtils.clamp(yawDiff * 0.2, -0.06, 0.06);
            const swayX = THREE.MathUtils.clamp(pitchDiff * 0.2, -0.06, 0.06);

            // Point right arm forward, tilting up/down with camera pitch, with bobbing/sway added
            rightArm.rotation.set(-1.4 + pitch + bobX + swayX, -0.25 + bobY + swayY, 0.3);
            rightForeArm.rotation.set(-0.9, -0.2, 0.0);
            if (rightHand) {
                rightHand.rotation.set(0.2, -0.4, 0.0);
            }
        }
    }
    ...
    // Make avatar model visible in POV, but hide the head and neck to keep first-person view clear
    if (playerModel && localBones) {
        playerModel.visible = true;
        if (localBones.head) localBones.head.scale.set(0.0001, 0.0001, 0.0001);
        if (localBones.neck) localBones.neck.scale.set(0.0001, 0.0001, 0.0001);
    }
    ...
    // Disable scissor test for full-screen rendering
    renderer.setScissorTest(false);

    // Hide head and neck for the local player's first-person view
    if (playerModel && localBones) {
        if (localBones.head) localBones.head.scale.set(0.0001, 0.0001, 0.0001);
        if (localBones.neck) localBones.neck.scale.set(0.0001, 0.0001, 0.0001);
    }

    // Update camera pitch relative to yaw
    if (camera) {
        camera.rotation.set(pitch, Math.PI, 0);
    }
...
function updatePlayerMovement(deltaTime) {
    ...
    if (isMoving) {
        ...
        const isSprinting = ((keys['w'] || keys['arrowup']) && (keys['q'] || keys['shift'])) || (isTouchDevice && mobileSprinting && joystickDir.lengthSq() > 0);
        const currentSpeed = isSprinting ? CONFIG.playerSpeed * 1.5 : CONFIG.playerSpeed;
...
function createProceduralFlashlight() {
    ...
    // Attach SpotLight and target directly to torchMesh so they align and move automatically!
    torchMesh.add(torchLight);
    torchMesh.add(torchTarget);
    torchLight.target = torchTarget;
    torchLight.position.set(0, 0, 0);
    torchTarget.position.set(0, 0, -1.0);
...
function createRemotePlayer(p) {
    ...
    // Use a high-visibility neon orange color for remote players to easily identify them
    const remoteColor = 0xffa500;
    const clonedScene = createProceduralStickman(remoteColor);
    group.add(clonedScene);

    // Scale and align remote player model dynamically
    const scaleFactor = CONFIG.playerHeight / 1.135;
    clonedScene.scale.set(scaleFactor, scaleFactor, scaleFactor);
    clonedScene.position.set(0, 0, 0);

    // Setup custom mecha cylinder flashlight
    const torchGeom = new THREE.CylinderGeometry(0.005, 0.005, 0.03, 8);
    const torchMat = new THREE.MeshBasicMaterial({ color: 0xffffff }); // Glowing white flashlight body
    const remoteTorch = new THREE.Mesh(torchGeom, torchMat);
    remoteTorch.rotation.x = Math.PI / 2;

    // Attach flashlight to right hand joint
    const rightHand = clonedScene.userData.bones.rHand;
    if (rightHand) {
        rightHand.add(remoteTorch);
        remoteTorch.position.set(0, -0.015, 0); // Position relative to palm joint
    }

    // Spotlight for flashlight beam
    spotlight = new THREE.SpotLight(0xffffff, 4.0, 1.2 * (CONFIG.playerHeight / 1.8), 0.38, 0.5, 1.0);
    ...
    // Add to remoteTorch
    remoteTorch.add(spotlight);
    
    const targetHelper = new THREE.Object3D();
    targetHelper.position.set(0, 0, -1.0); // Point forward along local -Z of flashlight
    remoteTorch.add(targetHelper);
    ...
    otherPlayers.set(p.id, {
        ...
        currentAction: null,
        bones: {
            head: clonedScene.userData.bones.head,
            rightArm: clonedScene.userData.bones.rShoulder,
            rightForeArm: clonedScene.userData.bones.rForearm,
            rightHand: clonedScene.userData.bones.rHand
        }
    });
}
...
function lerpRemotePlayers(deltaTime) {
    otherPlayers.forEach(p => {
        ...
        // Update remote player pitch smoothly
        p.currentPitch = THREE.MathUtils.lerp(p.currentPitch || 0, p.targetPitch || 0, 12 * deltaTime);

        const distMoved = prevPos.distanceTo(p.mesh.position);
        const speed = distMoved / deltaTime;
        const isMoving = speed > 0.005; // Moved more than 5mm in a second

        // Update remote player procedural stickman bones!
        if (p.model && p.model.userData && p.model.userData.bones) {
            const bones = p.model.userData.bones;
            
            // Advance walk cycle
            p.walkCycle = (p.walkCycle || 0) + deltaTime * 12 * (p.isSprinting ? 2.2 : 1.0);
            
            if (isMoving) {
                const swing = Math.sin(p.walkCycle);
                const swingDouble = Math.sin(p.walkCycle * 2);
                ...
                // Right arm (flashlight) follows pitch
                bones.rShoulder.rotation.x = -1.4 + p.currentPitch + swingDouble * 0.05;
                bones.rForearm.rotation.x = -0.9 + swingDouble * 0.02;
                bones.rHand.rotation.x = 0.2;
            } else {
                const breathe = Math.sin(Date.now() / 500);
                // Idle breath
                bones.lThigh.rotation.set(0, 0, 0.05);
                bones.rThigh.rotation.set(0, 0, -0.05);
                bones.lKnee.rotation.set(0, 0, 0);
                ...
                bones.rShoulder.rotation.set(-1.4 + p.currentPitch + breathe * 0.02, -0.25, 0.3);
                bones.rForearm.rotation.set(-0.9, -0.2, 0);
            ...
            // Always rotate head based on look pitch
            bones.head.rotation.x = p.currentPitch;
        }
...
function sendPlayerUpdate() {
    ...
    ws.send(JSON.stringify({
        ...
        isSprinting: ((keys['w'] || keys['arrowup']) && (keys['q'] || keys['shift'])) || mobileSprinting,
