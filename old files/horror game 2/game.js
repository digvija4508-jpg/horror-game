...
function init() {
    ...
    // --- Simple Cylinder Avatar (local player) ---
    {
        ...
        const bodyMat = new THREE.MeshStandardMaterial({ color: 0x1b1b22, roughness: 0.8, metalness: 0.1 });
        playerModel = new THREE.Mesh(bodyGeom, bodyMat);
        playerModel.castShadow = true;
        playerModel.receiveShadow = true;
        playerModel.position.set(0, bodyHeight / 2, 0); // feet at Y=0

        // Set up player group to rotate/translate cleanly
        playerGroup = new THREE.Group();
        playerGroup.add(playerModel);

        // Position first-person camera at eye level (95% of player height) and shift forward to the eyes
        camera.position.set(0, CONFIG.playerHeight * 0.95, -bodyRadius);
...
// No animation mixer needed for cylinder avatar
function updatePlayerAnimations(deltaTime) {
    // Cylinder avatar has no skeletal animations
}
...
function animate() {
    ...
    // Hide local cylinder in first-person view (we'll show it for third-person below)
    if (playerModel) {
    ...
    // Viewport: Full-screen First-Person POV - hide local cylinder
...
function createRemotePlayer(p) {
    ...
    // Simple cylinder body for remote player
    const bodyRadius = CONFIG.playerHeight * 0.12;
    const bodyHeight = CONFIG.playerHeight;
    const bodyGeom = new THREE.CylinderGeometry(bodyRadius, bodyRadius, bodyHeight, 16);
    const bodyMat = new THREE.MeshStandardMaterial({ color: 0x1b1b22, roughness: 0.8, metalness: 0.1 });
